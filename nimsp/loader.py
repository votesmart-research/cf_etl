...
def match(records_transformed: dict[int, dict[str, str]], records_ec: dict[int, dict[str, str]]):
    ...
    tb_config.scorers_by_column.SCORERS.update({'WRatio': lambda x, y:
                                                fuzz.WRatio(str(x).lower(), str(y.lower()))})
    ...
    records_matched, match_info = tb_matcher.match(
        update_func=lambda: p_bar.update(1))
...
def verify(records_matched: dict, records_finsource: dict, col_name):
    ...
    for row_i in records_verified.values():
        ...
        for row_j in records.group_by(records_finsource,
                                      {'code': str(row_i[col_name])}).values():
...
def main(records_transformed: dict) -> tuple[dict, dict]:
    ...
    election_years = {str(row['election_year'])
                      for row in records_transformed.values()}

    formatted_query_ec = query_election_candidates + \
        f"""
    ...
    formatted_query_nimsp = query_finsource_candidates + \
        f"""
        WHERE finsource_id = 4
        AND code IN ({",".join(nimsp_ids)})
        """

    records_nimsp = query_from_database(
        formatted_query_nimsp, vs_db_connection)
